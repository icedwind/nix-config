...
local M          = {
  metadata = {
    ...
    },
    {
      count = 0,
      pinned = true,
      icon = getIcon(nil, "vesktop", "vesktop"),
      id = 3,
      clients = {},
      class = "vesktop",
      exec = "vesktop",
    ...
    },
    {
      count = 0,
      pinned = true,
      icon = getIcon(nil, "thunar", "thunar"),
      id = 4,
      clients = {},
      class = "thunar",
      exec = "thunar",
  ...
  entries = {},
  classes = { "zen", "org.wezfurlong.wezterm", "discord", "spotify", "steam", "thunar" }
}
...
function M:showMenu(data)
  ...
  self.popup.y = 1080 - 90 - (50 * (#clients + 2))
  ...
  local windowInfo = wibox.widget {
    {
      {
        {
          {
            markup = "Print Window Info",
            font   = beautiful.font .. " 12",
            widget = wibox.widget.textbox,
          },
          widget = wibox.container.constraint,
    ...
    buttons = {
      awful.button({}, 1, function()
        for i, j in ipairs(clients) do
          naughty.notify{ title=j.name, text="Class: ".. j.class }
        end
      end)
    },
    widget = wibox.container.background,
  ...
  self.popupWidget:add(windowInfo)
